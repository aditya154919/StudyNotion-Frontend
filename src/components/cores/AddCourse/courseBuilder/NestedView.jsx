...
export default function NestedView({handleChangeEditSectionName}) {
  ...
  const handleDeleleSection = async (sectionId) => {
    const data ={sectionId:sectionId,courseId:course._id}
    console.log("FORMDATA",data)
    const result = await deleteSection(data,token)
    if (result) {
      dispatch(setCourse(result))
    }
    // console.log("Hellosd jchdsc")
    setConfirmationModal(null)
  }

  const handleDeleteSubSection = async (subSectionId, sectionId) => {
    const data = {subSectionId:subSectionId,sectionId:sectionId}
  ...
  return (
    <>
          ...
          <details key={section._id} open>
            ...
            <summary className="flex cursor-pointer items-center justify-between border-b-2 border-b-[#424854] py-2">
              ...
              <div className="flex items-center gap-x-3">
                ...
                >
                  <MdEdit className="text-xl cursor-pointer text-[#838894]" />
                </button>
                <button
                  onClick={() =>
              ...
              {(section.subSection || []).map((data) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <RiDeleteBin6Line className="text-xl cursor-pointer text-[#838894]" />
                    </button>
                  </div>
                </div>
              ))}
      ...
      {/* Confirmation Modal */}
      {confirmationModal ? (
        <ConfirmationModal modelData={confirmationModal} />
      ) : (
        <></>
      )}
    </>
